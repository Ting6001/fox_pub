...
def get_machine_id(cursor, tablename, dic):
...
def get_err(ary_id, ary_x, ary_y, col, subgroup=1, dic_no=None):
    ...
    dic_mr_unit = {}
    lst_mr = []
    lst_mr = [abs(v - u) for u, v in zip(ary_y, ary_y[1:])] # |N(i-1) - N(i)| moving range目前的值與前一點的值差距
    mr = np.array(lst_mr) 
...
# CL：avg(MR)
    ...
    tablename = 'machine_mean'
    col_mean = 'mean_' + col
    col_mr = 'mean_mr_' + col
    # ======================== update mean ========================
    if not dic_no is None:
        if get_machine_id(cursor, 'machine_mean', dic_no):
            sql = f'''update {tablename} set 
            e2 = {e2}, 
            d4 = {d4},
            {col_mean} = {x_bar}, 
            {col_mr} = {mr_bar}  
            where device_no = '{dic_no['device_no']}' and station_no = '{dic_no['station_no']}'  
            and robot_no = '{dic_no['robot_no']}' '''
        else:
            sql = f'''insert into {tablename} (device_no, station_no, robot_no, e2, d4, {col_mean}, {col_mr})
            values (
                '{dic_no['device_no']}', '{dic_no['station_no']}',  '{dic_no['robot_no']}', 
                {e2}, {d4}, {x_bar}, {mr_bar} 
            )'''
        logger.info(sql)
        cursor.execute(sql)
